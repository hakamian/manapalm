...
const CartSidebar: React.FC = () => {
  ...
  const handleCheckout = () => {
    ...
    const itemsWithPlan = cartItems.filter(item => item.paymentPlan);
    const hasInstallments = itemsWithPlan.length > 0;

    // Simplification: If mixed bag, strictly use the first plan or error out. 
    // For this release, assume single plan for the order if any exists.
    const primaryPlan = hasInstallments ? itemsWithPlan[0].paymentPlan : undefined;

    // Validate: if multiple items have different plans, we might need a block. 
    // For now, let's assume the user selects one plan per session or they are compatible.
    ...
    const newOrder: Order = {
      ...
      deeds: newDeeds,
      type: hasInstallments ? 'installment' : 'standard',
                      ...
                      <div className="flex-grow">
                        ...
                        <div className="flex justify-between items-center mt-3">
                          ...
                          <button
                            ...
                            className={`text-xs font-bold py-1.5 px-3 rounded-lg transition-all flex items-center gap-1 ${upsellState === 'success' ? 'bg-green-600 text-white' :
