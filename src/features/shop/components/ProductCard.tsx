    ...
    return (
        <>
            <div className={`group bg-gray-800 rounded-lg overflow-hidden shadow-lg transform relative border border-gray-700 transition-all duration-300 hover:-translate-y-2 ${isDigital ? 'hover:border-blue-500' : 'hover:border-green-500'}`}>
                ...
                {/* Impact Badge */}
                {product.impactValue && product.impactValue > 0 && (
                    <div className="absolute bottom-3 left-3 flex items-center bg-emerald-900/90 text-emerald-300 border border-emerald-500/50 text-xs font-bold px-2 py-1 rounded-full z-10 shadow-lg backdrop-blur-sm">
                        <SparklesIcon className="w-3 h-3 ml-1" />
                        <span>{product.impactValue} {product.impactUnit === 'tree' ? 'نخل' : product.impactUnit === 'hour' ? 'ساعت آموزش' : product.impactUnit === 'meal' ? 'وعده غذا' : 'واحد اثر'}</span>
                    </div>
                )}
