...
- **هدف:** رفع عدم تطابق شماره سفارش در پروفایل و پیامک، و بهبود تجربه کاربری در آیین کاشت.
- **اقدامات:**
  1. **Order ID Standardization:**
     - تغییر نمایش شماره سفارش در تمامی بخش‌های پروفایل (`OrdersTab.tsx` و `DashboardTab.tsx`) از ۶ رقم آخر به **۸ رقم اول** آیدی سفارش.
     - این تغییر باعث تطابق کامل شماره نمایش داده شده در سایت با شماره ارسال شده در پیامک تایید سفارش شد.
  2. **Deed Personalization UX (آیین کاشت):**
     - **Default Donor Name:** فیلد «نام هدیه‌دهنده» اکنون به صورت خودکار با نام کامل کاربر (اگر وارد شده باشد) پر می‌شود تا سرعت فرآیند خرید بالا برود.
     - **Empty Spiritual Owner:** فیلد «صاحب معنوی» در شروع کار عمداً خالی شد تا کاربر نیت قلبی خود را وارد کند.
     - **Context-Aware Messages:** پیاده‌سازی متون پیش‌فرض هوشمند در فیلد پیام نخل بر اساس نوع نخل انتخابی (مثلاً متن تبریک ازدواج برای نخل پیوند، متن یادبود برای نخل رویش دوباره و ...).
  3. **SMS Verification Fix:** اصلاح مجدد منطق ارسال پیامک برای جلوگیری از تکرار عبارت «ثبت شد» در متن نهایی.
- **نتیجه:** ابهام کاربر در مورد شماره سفارش برطرف شد و فرآیند شخصی‌سازی سند نخل روان‌تر و هوشمندتر گردید.
...
- **اقدامات:**
  ...
  4. **SMS Infrastructure Refactoring:** جداسازی کامل الگوهای پیامک ورود (`SMS_IR_OTP_TEMPLATE_ID`) و ثبت سفارش (`SMS_IR_ORDER_TEMPLATE_ID`) و اصلاح منطق ارسال برای جلوگیری از تکرار متن.
  5. **Deed UX Improvements:** تنظیم فیلد صاحب معنوی به صورت خالی در شروع کار و پیاده‌سازی متون پیش‌فرض هوشمند (Intelligent Defaults) متناسب با نوع نخل (ازدواج، تولد، یادبود و ...) در بخش نیت و پیام.
  6. **Correct Image Assets:** اصلاح نمایش تصاویر نخل‌ها در سبد خرید و صدور سند با جایگزینی آدرس‌های تستی (Picsum) با تصاویر اختصاصی نخلستان.
  7. **Timeline Stability:** افزودن محدودیت ۲ مگابایتی برای آپلود تصاویر خاطرات در `TimelineTab.tsx` و اصلاح منطق استخراج سند (Deed Extraction).
