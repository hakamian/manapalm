...
- **تاریخ:** ۱۴ دی ۱۴۰۳
- **هدف:** نمایش همزمان تصویر گرافیکی (پیش‌فرض) و تصویر واقعی (هاور) برای محصولات.
- **اقدامات:**
  1. **Schema Update:** اضافه کردن فیلد `realImage` به مدل `Product`.
  2. **Component Upgrade:** بازنویسی `ProductCard.tsx` با استایل Glassmorphism و افکت Hover برای نمایش تصویر واقعی.
  3. **Bug Fix:** رفع خطای بیلد مربوط به آیکون `ShoppingBagIcon` (جایگزینی با `ShoppingCartIcon`).
- **نتیجه:** کارت‌های محصول اکنون مدرن‌تر شده و قابلیت نمایش دو تصویر را دارند (منتظر ورود لینک‌های واقعی).
