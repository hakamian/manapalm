...
- **اقدامات:**
  ...
  4. **SMS Infrastructure Refactoring:** جداسازی کامل الگوهای پیامک ورود (`SMS_IR_OTP_TEMPLATE_ID`) و ثبت سفارش (`SMS_IR_ORDER_TEMPLATE_ID`) و اصلاح منطق ارسال برای جلوگیری از تکرار متن (ارسال تنها کد سفارش).
  5. **Donor Name UX:** تنظیم نام کاربر جاری به عنوان مقدار پیش‌فرض در فیلد «نام هدیه‌دهنده» در مودال آیین کاشت.
  ...
  7. **Timeline Stability:** افزودن محدودیت ۲ مگابایتی برای آپلود تصاویر خاطرات در `TimelineTab.tsx` و اصلاح منطق استخراج سند (Deed Extraction) در `CheckoutView.tsx`.
