...
export default function AuthModal({ isOpen, onClose, onLoginSuccess }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (data.user) {
          onLoginSuccess({ email: data.user.email, fullName: data.user.user_metadata?.full_name });
          onClose();
        }
      } else {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isLogin ? 'ورود' : 'ثبت‌نام'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 py-3 rounded-lg hover:bg-gray-50"
        >
