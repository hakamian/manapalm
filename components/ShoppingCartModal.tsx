    ...
    <Modal
      ...
      title={
        <div className="flex items-center gap-3">
          <span className="text-gray-900 dark:text-white">فرآیند تکمیل معنا</span>
        </div>
      }
    >
      <div className="w-[95vw] max-w-md min-h-[500px] flex flex-col">
          ...
          {steps.map((s, idx) => (
            <React.Fragment key={s.id}>
              <div className="relative flex flex-col items-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${activeStep >= s.id ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-200 dark:border-gray-700 text-gray-500'}`}>
              ...
              {idx < steps.length - 1 && (
                <div className={`flex-1 h-[2px] mx-2 transition-colors duration-300 ${activeStep > s.id ? 'bg-emerald-500' : 'bg-gray-200 dark:bg-gray-800'}`} />
              )}
            </React.Fragment>
          ))}
        </div>
        ...
        {cartItems.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center py-12">
            <div className="w-20 h-20 bg-gray-100 dark:bg-gray-800/50 rounded-full flex items-center justify-center mb-6 border border-gray-200 dark:border-white/5 shadow-inner">
              <ShoppingCartIcon className="w-10 h-10 text-gray-600" />
            </div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">سبد شما خالی است</h3>
            <p className="text-gray-500 text-sm mb-8 text-center px-8">نیت‌های معنادار خود را بکارید تا سفرِ رشد شما آغاز شود.</p>
            <button onClick={handleClose} className="bg-emerald-50 dark:bg-emerald-600/10 hover:bg-emerald-100 dark:hover:bg-emerald-600/20 text-emerald-600 dark:text-emerald-400 px-8 py-3 rounded-2xl border border-emerald-200 dark:border-emerald-500/20 transition-all font-bold">بازگشت به نخلستان</button>
          </div>
        ) : (
          <div className="flex-1 flex flex-col">
            ...
            {activeStep === 1 && (
              <div className="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-1 animate-fade-in">
                {cartItems.map((item) => (
                  <div key={item.id} className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/5 rounded-2xl">
                    <img src={item.image} className="w-16 h-16 rounded-xl object-cover" />
                    <div className="flex-1 min-w-0">
                      <h4 className="text-gray-900 dark:text-white font-bold text-sm truncate">{item.name}</h4>
                      <div className="flex items-center gap-4 mt-2">
                        <div className="flex items-center gap-2 bg-gray-100 dark:bg-black/40 rounded-lg px-2 py-1 border border-gray-200 dark:border-white/5">
                          <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="text-gray-500 hover:text-gray-900 dark:hover:text-white"><MinusIcon className="w-3 h-3" /></button>
                          <span className="text-gray-900 dark:text-white text-xs font-bold">{toFarsiDigits(item.quantity)}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="text-gray-500 hover:text-gray-900 dark:hover:text-white"><PlusIcon className="w-3 h-3" /></button>
                        </div>
            ...
            {activeStep === 2 && (
              <div className="space-y-6 animate-fade-in py-4">
                <div className="text-center mb-4">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white">صدای ریشه‌های شما</h3>
                ...
                <div className="grid grid-cols-1 gap-4">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-xl font-black text-gray-900 dark:text-white">{toFarsiDigits(impact.trees)} نخل</div>
                  ...
                  <div className="bg-blue-500/10 border border-blue-500/20 p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-xl font-black text-gray-900 dark:text-white">{toFarsiDigits(Math.ceil(impact.jobs))} نفر-روز</div>
                  ...
                  <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-xl font-black text-gray-900 dark:text-white">{toFarsiDigits(impact.co2)} کیلوگرم</div>
            ...
            {activeStep === 3 && (
              <div className="space-y-6 animate-fade-in py-8 text-center">
                ...
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">آماده‌ی میراث‌سازی؟</h3>
                <div className="bg-gray-50 dark:bg-gray-800/50 p-6 rounded-3xl border border-gray-200 dark:border-white/5 space-y-3">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">تعداد آیتم‌ها</span>
                    <span className="text-gray-900 dark:text-white font-bold">{toFarsiDigits(cartItems.length)} عدد</span>
                  </div>

                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">جمع کل برکت</span>
                    <span className="text-gray-900 dark:text-white font-bold">{formatPrice(totalPrice)} تومان</span>
                  </div>
                  ...
                  <div className="h-[1px] bg-gray-200 dark:bg-white/5 my-2" />
            ...
            {/* Footer Actions */}
            <div className="mt-auto pt-6 border-t border-gray-200 dark:border-white/5 space-y-3">
              ...
              {activeStep === 3 ? (
                ...
                <button
                  onClick={nextStep}
                  className="w-full bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-gray-900 dark:text-white py-4 rounded-2xl transition-all font-bold flex items-center justify-center gap-2 group"
                >
