...
export const dbAdapter = {
    ...
    async getUserById(id: string): Promise<User | null> {
        ...
        // Note: The new schema uses TEXT for id, so 'user_1' IS valid in DB if inserted.
    ...
    async saveUser(user: User): Promise<void> {
        ...
        // Separate core columns from metadata
        // These fields exist as columns in 'profiles' table
        const { 
            id, email, fullName, name, phone, avatar, 
            points, manaPoints, level, 
            isAdmin, isGuardian, isGroveKeeper, 
            joinDate, 
            ...metadata // Everything else goes into JSONB
        } = user;
        ...
        const profileData = {
            ...
            phone: phone,
            avatar_url: avatar, // Mapping app 'avatar' to DB 'avatar_url'
            points: points,
            ...
            metadata: metadata // Store rest of the data
        };
    ...
    async saveAgentLog(log: AgentActionLog): Promise<void> {
        ...
        await supabase!.from('agent_tasks').insert({
            id: isUUID(log.id) ? log.id : undefined, // Let DB gen UUID if needed
            type: log.action,
