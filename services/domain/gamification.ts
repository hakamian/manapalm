...
export const BARKAT_LEVELS = [
    { name: 'جوانه', points: 0, manaThreshold: 0, description: "آغاز سفر: پتانسیل نهفته" },
    { name: 'نهال', points: 2500, manaThreshold: 200, description: "ریشه‌دوانی: شروع مشارکت و یادگیری" },
    { name: 'درخت جوان', points: 10000, manaThreshold: 1500, description: "قد کشیدن: تعهد جدی به رشد و معنا" },
    { name: 'درخت تنومند', points: 50000, manaThreshold: 8000, description: "سایه‌گستری: تاثیرگذاری عمیق بر جامعه" },
    { name: 'استاد کهنسال', points: 200000, manaThreshold: 30000, description: "جاودانگی: تبدیل شدن به راهنما و اسطوره" },
];
...
export const getLevelForPoints = (barkatPoints: number, manaPoints: number = 0) => {
    ...
    for (const level of BARKAT_LEVELS) {
...
export const calculateDailyChestReward = (currentStreak: number): DailyChestReward => {
    ...
    const streakBonus = Math.min(currentStreak * 0.1, 2.0);

    if (rand < 0.60) {
        const basePoints = Math.floor(Math.random() * 50) + 50;
        const finalPoints = Math.floor(basePoints * (1 + streakBonus));
        return { type: 'barkat', amount: finalPoints, message: 'برکت امروز شما', bonusMultiplier: 1 + streakBonus };
    } else if (rand < 0.90) {
        const basePoints = Math.floor(Math.random() * 20) + 10;
        const finalPoints = Math.floor(basePoints * (1 + streakBonus));
        return { type: 'mana', amount: finalPoints, message: 'نور آگاهی امروز', bonusMultiplier: 1 + streakBonus };
    } else {
        const rewardType = Math.random() > 0.5 ? 'barkat' : 'mana';
        const basePoints = rewardType === 'barkat' ? 500 : 100;
        return { type: rewardType, amount: basePoints, message: 'گنجینه حماسی!', item: 'کارت تخفیف ویژه', bonusMultiplier: 1 };
    }
};
